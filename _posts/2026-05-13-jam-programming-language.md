...
**Edit, Jul 2026:** after the discussion on [Lobsters](https://lobste.rs/s/r0xrm0/jam_programming_language), I fixed a few things in this post: the Rust FFI comparison was unfair (and partly wrong about Rust's ABI), the first drop example deserved an honest note about the descriptor it returns, and a couple of safety claims were stated more broadly than they should have been.
...
Jam is not alone in this corner. [Hylo](https://www.hylo-lang.org/), from the authors of that same paper, is built on mutable value semantics too, and Swift has been leaning into value semantics for years.
...
One thing though: `useFile` returns `f.fd` *after* drop has closed it, so the caller ends up holding a dead descriptor. The example is demonstrating when drop fires, not claiming the compiler tracks raw resource handles. An `i32` is just a number; once you copy it out of the struct, Jam has no idea it aliases the file, the same way Rust loses track of a leaked `as_raw_fd()`. If you need the handle to outlive the scope, you return the `File` itself and let ownership move to the caller.
...
Rust's default `struct` layout is deliberately unstable: it can change between compiler versions, so it's not something another language can rely on. (The first version of this post overstated this, and Lobsters rightly called it out: `&T`, `&mut T`, `Box<T>`, and `Option`s of them all *do* have defined ABIs, and lifetimes don't exist at the binary level at all. The instability is about default layout and std's types, not about references.) So exporting a C API from Rust means marking the types that cross with `#[repr(C)]`, the functions with `#[no_mangle] extern "C"`, and touching the incoming pointer inside `unsafe`:
...
One `repr` annotation per crossing type, the `extern "C"` spelling, and one `unsafe` deref per entry point. It's still a second dialect you switch into at the boundary, and it still scales with every type and function you export, `cbindgen` and `abi_stable` exist because people got tired of writing it by hand, but it's ceremony, not a cliff.
...
The `mut Counter` parameter lowers to exactly that `Counter *`: the mode-aware ABI passes `mut` as a pointer to caller-owned storage, which is what a C caller already holds. No `repr` annotation, no raw-pointer cast, no `unsafe` keyword. But let me be straighter about that last part than I was in the first version of this post: the danger doesn't disappear. A C caller can hand `counterAdd` a null or dangling pointer, and Jam can't verify the far side of that call any more than Rust can. Rust makes you spell `unsafe` at the entry point precisely to mark that assumption; Jam today leaves it implicit. What `export` saves you is the ceremony, not the risk at the boundary. Inside the function, the body is ordinary Jam: drop still fires, init analysis still runs, and the call-site exclusivity rule still holds.
...
One honest caveat: at the `extern` line you are talking to C, and C's rules win. `extern` functions follow the C ABI literally, so the parameter-mode machinery from the last section doesn't apply across that boundary, raw pointers (`*const[] u8`, `*mut[] u8`) are how you hand C a buffer, and what C does with that pointer is C's problem. Jam doesn't pretend to verify the far side of an FFI call.

And yes, this cuts against the grep-for-`unsafe` story from the `Maybe(T)` section, an `extern` call asserts runtime invariants with no `unsafe` anywhere in its name. To be honest I am conflicted about this yet, and might change for v0.1. What it does give you is that *your* side stays safe by default, and exposing a Jam library across the C ABI costs nothing extra: no shim layer, no second unsafe API mirroring the safe one, no annotation tax on every type. You write the function once, safely, and `export` it.
